...
describe("part 1 — matchFlow (subsequence)", () => {
  ...
  it("respects a step's method constraint", () => {
    const getOrder = matchFlow(CHECKOUT, [
      rec("GET", "/cart"),
      rec("GET", "/checkout"),
      rec("GET", "/api/order"),
    ]);
  ...
  it("matches a trailing wildcard step and ignores query strings", () => {
    const flow: Flow = {
      name: "browse",
      steps: [{ path: "/product/*" }, { path: "/checkout" }],
    };
    const m = matchFlow(flow, [
      rec("GET", "/product/42?ref=home"),
      rec("GET", "/checkout?step=1"),
    ]);
    expect(m.completed).toBe(true);
  });
