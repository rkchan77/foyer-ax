...
const session = (records: RequestRecord[]): Session => ({
  ip: "203.0.113.5",
  records,
});
...
describe("detectFriction — friction signals", () => {
  ...
  it("detects a 4xx cluster", () => {
    ...
    expect(
      detectFriction(s).findings.some((f) => f.kind === "4xx-cluster"),
    ).toBe(true);
  });
  ...
  it("detects an auth wall (repeated 401/403)", () => {
    ...
    expect(detectFriction(s).findings.some((f) => f.kind === "auth-wall")).toBe(
      true,
    );
  });
  ...
  it("detects abandonment when the session ends on an error", () => {
    ...
    expect(
      detectFriction(s).findings.some((f) => f.kind === "abandonment"),
    ).toBe(true);
  });
