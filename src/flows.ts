...
export function matchFlow(flow: Flow, records: RequestRecord[]): FlowMatch {
  ...
  for (const record of records) {
    ...
    const methodMatch =
      !flow.steps[flowStep].method ||
      flow.steps[flowStep].method?.toUpperCase() ===
...
export function computeFunnel(
  ...
  for (const mediumStat of Object.values(stats)) {
    mediumStat.completionRate =
      mediumStat.total > 0 ? mediumStat.completed / mediumStat.total : 0;
  }
